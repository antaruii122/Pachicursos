...
**Parte D — Integración Vimeo segura** (adelantada parcialmente; falta lo más importante, el endpoint de reproducción):
...
- [ ] **Falta lo central de la Parte D**: el endpoint server-side que valida acceso (gratis o compra pagada) y recién ahí pide el embed real a Vimeo — hoy el reproductor (Parte C) sigue mostrando un placeholder visual, nunca pide el video real. Sin esto no hay nada que reproducir de verdad todavía.
- No probado con un video real (mismo bloqueo que Parte C: falta que Ricardo tenga una cuenta admin).
